...
class UIHeader extends Component {
	static propTypes = {
		theme: PropTypes.oneOf(["default", "primary", "image", "grey"]),
	...
	fixHeight = () => {
		if (isIPhoneWithMonobrow() && this.props.isModal) {
			return 0
		}
		...
		if (themeImage) {
			return (
				<ImageBackground
					...
					{this.props.enableStatusBar && <View style={{ height: STATUS_BAR_HEIGHT, backgroundColor: this.props.statusBarColor ? this.props.statusBarColor : this.getThemeConfig("statusBarBackground") }} />}
					<View style={[styles.container, styleContainer, this.props.styleContainer]}>
						{leftElements && <View style={[styles.leftContainer, { marginBottom: this.fixHeight() }]}>
							{leftElements}
						</View>}
						{centerElements && <View style={[styles.centerContainer, { marginBottom: this.fixHeight() }]}>
							{centerElements}
						</View>}
						{rightElements && <View style={[styles.rightContainer, { marginBottom: this.fixHeight() }]}>
							{rightElements}
						</View>}
					</View>
				</ImageBackground>
